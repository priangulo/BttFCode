...
import bttf.Feature;
import bttf.FeatureBoundsCalculation;
...
public class BttFMain extends JFrame {
	...
	private String set_element_options(Element current_element){
		...
		try{
			FeatureBoundsCalculation fbcalc = new FeatureBoundsCalculation(this.partition);
			FeaturePossibility possibilities = fbcalc.get_element_possibilities(current_element, this.parent_feature, this.recursive_partition);
			refresh_buttons(possibilities.getButton_options(), true);
			
			boolean pickForMe = false; //true;
			if(pickForMe){
				System.out.println("Pick for me!");
				Feature autoFeature = possibilities.getFeature_options().stream()
